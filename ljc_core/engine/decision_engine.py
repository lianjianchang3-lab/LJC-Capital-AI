...
from ljc_core.engine.main_force_engine import MainForceEngine


class DecisionEngine:
    def __init__(self):
        self.data = DataEngine()
        self.mf = MainForceEngine()
    ...
    def run(self):
        ...
        for _, s in watch.iterrows():
            m = self.mf.summary(s["code"], mf_df)
            ...
            rows.append({
                ...
                "risk": risk, "lia": lia, "confidence": confidence,
                "pool": pool, "action": action, "priority": priority,
                "close": close, "change_pct": change_pct, **m,
        ...
        return {
            ...
            "diamond": diamond, "opportunity": opportunity, "watch": watch_rows,
            "alerts": alerts or ["暂无重大预警"], "rows": rows,
        }
