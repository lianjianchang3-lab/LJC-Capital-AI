...
try:
    ...
    with btabs[0]:
        st.subheader("Build031-040 Master Strategy V3")
        result = master_v3.generate()
        market = result.get("market", {})
        a, b, c, d = st.columns(4)
        a.metric("市场", market.get("regime", "N/A"))
        b.metric("平均LIA", market.get("avg_lia", "N/A"))
        c.metric("强势数", market.get("strong_count", "N/A"))
        d.metric("市场宽度", market.get("breadth", "N/A"))
        st.warning(result.get("warning", ""))
        st.dataframe(pd.DataFrame(result.get("top_plans", [])), use_container_width=True, hide_index=True)

    with btabs[1]:
        st.subheader("Build032-035 Entry / Exit Plan")
        st.dataframe(pd.DataFrame(entry_exit.plans()), use_container_width=True, hide_index=True)

    with btabs[2]:
        st.subheader("Build041-045 AI Investment Score V3")
        st.dataframe(score_v3.table(), use_container_width=True, hide_index=True)

    with btabs[3]:
        st.subheader("Build046-050 Selection Center")
        scan = selection_center.scan()
        st.json({"status": scan.get("status")})
        st.markdown("#### Top 10")
        st.dataframe(pd.DataFrame(scan.get("top10", [])), use_container_width=True, hide_index=True)
        st.markdown("#### Main Wave")
        st.dataframe(pd.DataFrame(scan.get("main_wave", [])), use_container_width=True, hide_index=True)
        st.markdown("#### Leaders")
        st.dataframe(pd.DataFrame(scan.get("leaders", [])), use_container_width=True, hide_index=True)
    ...
    with btabs[5]:
        st.subheader("Build061-070 Daily Workbench")
        plan = wb.daily_plan()
        st.json({"time": plan.get("time")})
        st.markdown("#### Strategy")
        st.json(plan.get("strategy", {}))
        st.markdown("#### Portfolio")
        st.json(plan.get("portfolio", {}))

    with btabs[6]:
        st.subheader("Markdown Report Export")
        st.code(wb.report_markdown(), language="markdown")

except Exception as e:
    st.error(f"V8.1 Build031-070 Strategy Workbench 加载失败：{e}")

# ==============================
# V8.1 Build071-100 Official Commander
# ==============================
try:
    st.divider()
    st.header("🚀 V8.1 Build071-100 Official Commander")

    commander = CommanderCenter()
    trading_plan = TradingPlanEngine()
    alert_center = AlertCenter()
    official_validation = OfficialValidation()
    ...
    with otabs[0]:
        st.subheader("Build086-090 Commander Center")
        snap = commander.snapshot()
        a,b,c,d = st.columns(4)
        a.metric("市场", snap.get("market", {}).get("regime"))
        b.metric("总仓位", snap.get("suggested_total_position"))
        c.metric("计划数", snap.get("top_plan_count"))
        d.metric("预警数", snap.get("alert_count"))
        st.success(snap.get("final_decision"))
        st.warning(snap.get("risk_note"))

    with otabs[1]:
        st.subheader("Build076-080 Trading Plan Center")
        plan = trading_plan.generate()
        st.json({"market_regime": plan.get("market_regime"), "suggested_total_position": plan.get("suggested_total_position"), "note": plan.get("note")})
        st.dataframe(pd.DataFrame(plan.get("plans", [])), use_container_width=True, hide_index=True)

    with otabs[2]:
        st.subheader("Build081-085 AI Alert Center")
        alerts = alert_center.scan()
        st.metric("Alert Count", alerts.get("alert_count"))
        st.dataframe(pd.DataFrame(alerts.get("alerts", [])), use_container_width=True, hide_index=True)

    with otabs[3]:
        st.subheader("Build091-095 Official Validation")
        v = official_validation.validate()
        st.metric("Overall", v.get("overall"))
        st.dataframe(pd.DataFrame(v.get("checks", [])), use_container_width=True, hide_index=True)

    with otabs[4]:
        st.subheader("Build096-099 Morning Brief")
        snap = commander.snapshot()
        st.markdown(f"""
### LJC Morning Brief

- 市场状态：{snap.get('market', {}).get('regime')}
- 建议总仓位：{snap.get('suggested_total_position')}
- 当前预警数：{snap.get('alert_count')}
- 最终判断：{snap.get('final_decision')}
- 风险提示：{snap.get('risk_note')}
""")

    with otabs[5]:
        st.subheader("Build100 V8.1 Official Candidate")
        v = official_validation.validate()
        if v.get("pass"):
            st.success("LJC Capital AI Pro V8.1 已达到 Official Candidate 状态。")
        else:
            st.error("V8.1 尚未全部通过 Official Validation。")
        st.info("后续建议进入 V9.0：实时行情 Provider、云端同步、自动预警推送。")

except Exception as e:
    st.error(f"V8.1 Build071-100 Official Commander 加载失败：{e}")
