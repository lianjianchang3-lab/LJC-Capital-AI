...
from engine_v33.decision_os import V33DecisionOS
...
def render_home():
    st.set_page_config(page_title="LJC Capital AI Pro V3.3", page_icon="🚀", layout="wide")
    _style()
    data = V33DecisionOS().run()
    wr = data["war_room"]

    st.markdown("### 🚀 LJC Capital AI Pro V3.3")
    st.caption("Data Engine Edition｜CSV数据驱动｜主力资金纵向跟踪")
    ...
    st.markdown("#### 📊 数据文件")
    with st.container(border=True):
        st.caption("当前页面从 data/watchlist.csv、data/main_force_daily.csv、data/stock_daily.csv 读取。以后只要更新CSV，手机页面会跟着变。")
